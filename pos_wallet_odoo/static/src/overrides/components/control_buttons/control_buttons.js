...
patch(BaypasPayment.prototype, {
  ...
  async checkMidtransPayment() {
    const order = this.pos.get_order();
    const midtransPayments = order.payment_ids
      .map((paymentline) => {
        const paymentMethodName =
          paymentline.payment_method_id.name.toLowerCase();
        if (
    ...
    return midtransPayments;
  },

  async generateMidtransToken(orderData) {
    try {
      const response = await rpc("/pos/midtrans/create_transaction", {
      ...
      if (response.success) {
        ...
        throw new Error(response.error || "Failed to generate Midtrans token");
      }
    } catch (error) {
  ...
  prepareMidtransOrderData() {
    const order = this.pos.get_order();
    const orderLines = order.get_orderlines();
    const customer = order.get_partner();

    // Generate unique order ID
    const orderId = `POS-${order.name || order.uid}-${Date.now()}`;

    // Prepare item details
    const itemDetails = orderLines.map((line) => {
      ...
      try {
        ...
        price = line.get_unit_price
          ? line.get_unit_price()
          : line.price_unit || line.price || 0;
      } catch (error) {
        console.error("Error preparing item details:", error);
      }

      return {
        id: line.id || `item-${Date.now()}`,
    ...
    // Calculate totals
    const grossAmount = Math.round(order.get_total_with_tax());
    ...
    const customerDetails = customer
      ? {
          ...
          phone: customer.phone || customer.mobile || "081234567890",
        }
      : {
  ...
  async showMidtransPopup(snapToken) {
    return new Promise((resolve, reject) => {
      if (!window.snap) {
        this.loadMidtransScript()
          .then(() => {
            this.openSnapPopup(snapToken, resolve, reject);
          })
          .catch(reject);
      } else {
        this.openSnapPopup(snapToken, resolve, reject);
  ...
  loadMidtransScript() {
    return new Promise((resolve, reject) => {
      ...
      script.setAttribute("data-client-key", "SB-Mid-client-PVz7kR_bNS6D7zVK"); // Ganti dengan client key mu
  ...
  openSnapPopup(snapToken, resolve, reject) {
  ...
  async verifyMidtransPayment(transactionId, orderId) {
    try {
      const response = await rpc("/pos/midtrans/verify_payment", {
  ...
  async clickSetSubTotal() {
    ...
    // Cek payment method
    const dompetSantriPayments = await this.checkPaymentMethod();
    const midtransPayments = await this.checkMidtransPayment();

    if (dompetSantriPayments.length > 0) {
      // Handle Dompet Santri payment (existing logic)
      const currentOrder = this.pos.get_order();
      const client = currentOrder.get_partner();
      if (!client) {
        customModal.showModal({
    ...
    } else if (midtransPayments.length > 0) {
      // Handle Midtrans payment
      try {
        const orderData = this.prepareMidtransOrderData();
        const snapToken = await this.generateMidtransToken(orderData);

        const paymentResult = await this.showMidtransPopup(snapToken);

        if (paymentResult.status === "success") {
          // Verify payment
          const verification = await this.verifyMidtransPayment(
            paymentResult.transaction_id,
            paymentResult.order_id
          );

          if (verification.success) {
            // Payment berhasil, lanjutkan proses order
            customModal.showModal({
              title: "Pembayaran Berhasil",
              body: `Pembayaran dengan ${paymentResult.payment_type} berhasil. Transaction ID: ${paymentResult.transaction_id}`,
              type: "success",
            });

            // Save transaction details ke order
            this.saveMidtransTransactionDetails(paymentResult);

            this.validateOrder();
          } else {
            customModal.showModal({
              title: "Verifikasi Gagal",
              body: "Pembayaran tidak dapat diverifikasi. Silakan coba lagi.",
              type: "error",
            });
          }
          ...
          customModal.showModal({
            ...
            body: "Pembayaran sedang diproses. Silakan tunggu konfirmasi.",
            type: "warning",
          });
        }
      } catch (error) {
        ...
        if (error.status === "closed") {
          customModal.showModal({
            ...
            body: "Pembayaran dibatalkan oleh pengguna.",
            type: "warning",
          });
        } else {
          customModal.showModal({
  ...
  saveMidtransTransactionDetails(paymentResult) {
    const order = this.pos.get_order();

    if (!order.midtrans_transaction) {
      order.midtrans_transaction = {};
    }
    ...
    console.log("Midtrans transaction saved:", order.midtrans_transaction);
  },
