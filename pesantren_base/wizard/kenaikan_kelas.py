...
class KenaikanKelas(models.Model):
        ...
        try:
            ...
            if self.status == 'naik':
                ...
                if not self.next_class:
                    raise UserError("Kelas selanjutnya tidak ditentukan untuk proses kenaikan kelas!")
                ...
                if santri_tidak_naik:
                    ...
                    kelas_tinggal_kelas = self._find_or_create_tinggal_kelas(tahun_ajaran_berikutnya)
    ...
    def _find_or_create_tinggal_kelas(self, tahun_ajaran_berikutnya):
        ...
        try:
            ...
            if self.kelas_id.name.jurusan_id:
                domain.append(('name.jurusan_id', '=', self.kelas_id.name.jurusan_id.id))
            
            kelas_tinggal_kelas = self.env['cdn.ruang_kelas'].search(domain, limit=1)
            ...
            if not kelas_tinggal_kelas:
                ...
                _logger.info(f"Berhasil membuat kelas tinggal kelas baru: {kelas_tinggal_kelas.nama_kelas}")
            
            return kelas_tinggal_kelas
            
        except Exception as e:
            _logger.error(f"Gagal mencari/membuat kelas tinggal kelas: {str(e)}")
            return False
    ...
    #                     'aktif_tidak': 'tidak',
    #                 })
    #                 message += f"✓ Kelas {self.kelas_id.nama_kelas} dinonaktifkan (semua siswa lulus)\n"
    ...
    #                 'aktif_tidak': 'aktif',
    #                 'keterangan': (
    #                     f"Program *Tinggal Kelas* berasal dari kelas {self.kelas_id.nama_kelas}"
    #                     if self.kelas_id.nama_kelas
    #                     else "Program *Tinggal Kelas*. Informasi kelas sebelumnya belum tersedia."
    #                 )
    #             })
    #             _logger.info(f"Berhasil membuat kelas tinggal kelas baru: {kelas_tinggal_kelas.nama_kelas}")
    ...
    #             if self.status == 'naik':
    #                 # Siswa naik kelas ke tingkat selanjutnya
    #                 if self.next_class:
    ...
    #             # Hapus santri yang tidak dicentang dari kelas
    #             for siswa in santri_tidak_naik:
    ...
    #                     message += f"Siswa {siswa.name} tidak lulus - dihapus dari kelas\n"
    #                 else:  # Status naik tapi tidak naik = tidak naik kelas
    #                     siswa.write({
    #                         'ruang_kelas_id': False,  # Hapus dari kelas
    #                     })
    #                     count_siswa_tidak_naik += 1
    #                     message += f"Siswa {siswa.name} tidak naik kelas - dihapus dari kelas\n"
                
    #             # Hapus santri yang tidak naik dari siswa_ids di kelas
    #             if santri_tidak_naik:
    #                 self.kelas_id.write({
    #                     'siswa_ids': [(3, siswa.id) for siswa in santri_tidak_naik]
    #                 })
    #                 message += f"Menghapus {len(santri_tidak_naik)} siswa dari daftar siswa kelas {self.kelas_id.nama_kelas}\n"
                
    #             if self.status == 'lulus':
    #                 message += f"Total {len(santri_tidak_naik)} siswa tidak lulus (dihapus dari kelas)\n"
    #             else:
    #                 message += f"Total {len(santri_tidak_naik)} siswa tidak naik kelas (dihapus dari kelas)\n"
         
         
    #         # ===== UPDATE DATA KELAS YANG SUDAH ADA =====
    #         if santri_naik and self.status == 'naik':
    #             if len(santri_naik) == len(self.partner_ids):
    #                 # Semua siswa naik
    #                 if self.next_class:
    #                     self.kelas_id.write({
    ...
    #                         'walikelas_id': self.walikelas_id.id,
    #                     })
    #                     message += f"\nKelas {self.kelas_id.nama_kelas} berhasil diupdate ke {self.next_class.name} untuk tahun ajaran {tahun_ajaran_berikutnya.name} (semua siswa naik)\n"
    ...
    #                 else:
    #                     message += "\nGagal naik kelas - kelas selanjutnya tidak ditentukan\n"
    #             else:
    #                 # Hanya sebagian siswa naik kelas
    #                 if self.next_class:
    #                     self.kelas_id.write({
    #                         'name': self.next_class.id,
    #                         'tahunajaran_id': tahun_ajaran_berikutnya.id,
    ...
    #                     # Hapus siswa yang tidak naik dari kelas ini
    #                     self.kelas_id.write({
    #                         'siswa_ids': [(3, siswa.id) for siswa in santri_tidak_naik]
    #                     })
    #                     message += f"\nSebagian siswa naik kelas - {len(santri_tidak_naik)} siswa dihapus dari kelas {self.kelas_id.nama_kelas}\n"
    ...
    #                     'aktif_tidak': 'tidak',
    #                 })
    #                 message += f"\nKelas {self.kelas_id.nama_kelas} dinonaktifkan karena semua siswa lulus\n"
    ...
    #             else:
    #                 # Hanya sebagian siswa lulus
    #                 # Hapus siswa yang tidak lulus dari kelas
    #                 self.kelas_id.write({
    #                     'siswa_ids': [(3, siswa.id) for siswa in santri_tidak_naik]
    #                 })
    #                 message += f"\nSebagian siswa lulus - {len(santri_tidak_naik)} siswa yang tidak lulus dihapus dari kelas {self.kelas_id.nama_kelas}\n"
    ...
    #     }
            ...
            # # ===== UPDATE DATA KELAS YANG SUDAH ADA =====
            # # Update tahun ajaran kelas saat ini jika ada siswa yang naik
            # if santri_naik and self.status == 'naik':
            #     # Update tahun ajaran kelas saat ini ke tahun berikutnya
            #     # karena siswa yang naik sudah dipindah ke kelas baru
            #     if len(santri_naik) == len(self.partner_ids):
            #         # Semua siswa naik, nonaktifkan kelas lama
            #         self.kelas_id.write({
            #             'aktif_tidak': 'aktif',
            #             'tahunajaran_id': tahun_ajaran_berikutnya.id,
            #             'name': self.next_class.id,
            #         })
            #         message += f"\nKelas {self.kelas_id.nama_kelas} dinonaktifkan (semua siswa naik kelas)\n"
            #     # else:
            #     #     # Sebagian siswa naik, update data kelas untuk tahun berikutnya
            #     #     # tapi tetap aktif untuk siswa yang tersisa
            #     #     remaining_students = self.partner_ids - santri_naik
            #     #     if remaining_students:
            #     #         self.kelas_id.write({
            #     #             'tahunajaran_id': tahun_ajaran_berikutnya.id,
            #     #             'name': self.next_class.id,
            #     #         })
            #     #         message += f"\nKelas {self.kelas_id.nama_kelas} diperbarui untuk tahun ajaran berikutnya\n"
            #     #         message += f"Siswa yang tersisa di kelas: {len(remaining_students)}\n"
            
            # elif self.status == 'lulus':
            #     # Untuk status lulus, nonaktifkan kelas jika semua siswa sudah diproses
            #     if len(santri_naik) == len(self.partner_ids):
            #         self.kelas_id.write({
            #             'aktif_tidak': 'tidak',
            #             # 'tahunajaran_id': tahun_ajaran_berikutnya.id
            #         })
            #         message += f"\nKelas {self.kelas_id.nama_kelas} dinonaktifkan (semua siswa lulus)\n"
            #     # else:
            #     #     # Update tahun ajaran untuk siswa yang tersisa
            #     #     self.kelas_id.write({
            #     #         'tahunajaran_id': tahun_ajaran_berikutnya.id
            #     #     })
            #     #     message += f"\nKelas {self.kelas_id.nama_kelas} diperbarui untuk tahun ajaran berikutnya\n"
    ...
    #                 # Siswa naik kelas ke tingkat selanjutnya
    #                 if self.next_class:
    #                     # Buat kelas baru untuk siswa yang naik
    #                     new_kelas_vals = {
    #                         'name': self.next_class.id,
    #                         'tahunajaran_id': tahun_ajaran_berikutnya.id,
    ...
    #                         'aktif_tidak': 'aktif',
    #                         'status': 'konfirm',
    #                         # Copy field lain yang diperlukan dari kelas lama
    #                         'angkatan_id': self.kelas_id.angkatan_id.id if self.kelas_id.angkatan_id else False,
    #                         'jenjang': self.kelas_id.jenjang,
    #                     }
                        
    #                     # Cek apakah kelas baru sudah ada
    #                     existing_new_kelas = self.env['cdn.ruang_kelas'].search([
    #                         ('name', '=', self.next_class.id),
    #                         ('tahunajaran_id', '=', tahun_ajaran_berikutnya.id),
    #                     ], limit=1)
                        
    #                     if existing_new_kelas:
    #                         kelas_tujuan = existing_new_kelas
    #                         message += f"Menggunakan kelas existing: {kelas_tujuan.nama_kelas}\n"
    #                     else:
    #                         kelas_tujuan = self.env['cdn.ruang_kelas'].create(new_kelas_vals)
    #                         message += f"Membuat kelas baru: {kelas_tujuan.nama_kelas}\n"
                        
    #                     # Update siswa yang naik kelas
    #                     for siswa in santri_naik:
    #                         siswa.write({
    #                             'ruang_kelas_id': kelas_tujuan.id,
    #                             'tahunajaran_id': tahun_ajaran_berikutnya.id,
    #                         })
    #                         count_siswa_naik += 1
